...
if __name__ == "__main__":
    ...
    for r in radi:
        s = general.sphere(r, padding=20)

        imgdata = vtk_functions.arr_to_imgdata(s)
        imgdata = vtk_functions.gauss_imgdata(imgdata, 5)
        pd = vtk_functions.imgdata_to_pd(imgdata)

        vol = vtk_functions.get_volume(pd)
        sa = vtk_functions.get_surface_area(pd)

        vols.append(vol)
        sas.append(sa)

        print("R: %s \t vol: %s \t sa: %s" % (r, vol, sa))
